...
net = caffe.Classifier(model_def, model_weights, mean=mean_image, channel_swap=(2, 1, 0), raw_scale=255, image_dims=(256, 256))
...
for i in range(start_num, last_num):
	...
	if len(row) > 0:
		...
		if (counter % counter_num == 0):
			...
			geograph_data_tosave.to_csv(
				code_folder + "results/transfer_learning/" + model_folder + 'geograph_data_predictions_RESNET_' + str(gpu_id) + '_' + str(counter) + '_1strun.csv')
			print(os.getcwd())
			print("saving file...")
